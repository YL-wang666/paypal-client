...
class PhoneBlogs extends Component {
    constructor(props) {
        super(props);
        this.state = { 
        ...
        return (
            <div>
                ...
                <div className="Pblogs-text-mobule">
                    <div className="Pblogs-info-mobule">
                        <h2>Table Of Contents</h2>
                        {
                            this.state.blogList.map((value , key)=>{
                                return <Link rel="noopener noreferrer" rel="noreferrer" to={"/blog_info?bn="+value.id}>
                                            <h3 value={key}>{value.title}</h3>
                                        </Link>
                            })
                        }
                        <div style={{marginTop:"15%",width:"90%",textAlign:"center"}}>
                    ...
                    <div className="Pblogs-product-mobule">
                        {
                            this.state.productArr.map((value , key)=>{
                                return  <div style={{marginTop:"6%"}} className="Pblogs-product-info">
                                            <div className="Pblogs-product-off-label">$50 OFF</div>
                                            <img value={key + 1} onClick={this.turnDetailPage} style={{width:"180px",height:"202px"}} src={require("../../assets/images/" + value.img)} />
                                            <p>{value.title}</p>
                                            <div className="Pblogs-product-func">{value.func}</div>
