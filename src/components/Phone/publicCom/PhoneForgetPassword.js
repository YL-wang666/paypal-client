...
class PhoneForgetPassword extends Component {
    constructor(props) {
        super(props);
        this.state = { 
            signUpMessage:"",
...
//样式就和Login用一样的，所以className同样命名
    render() {
        return (
            <form className="PLoginForm">
                <h2 className="Psign-logo">Jendey</h2>
                <hr />
                
                <div className="PSignUpFormText">
                    <div className="Psign-title-mobule">
                        <h2 className="Psign-title">Reset Password</h2>
                        <div className="Psign-link-text"><i onClick={this.changePage}> Login </i></div>
                    </div>
                    <div>
                        <label>Account:</label><br />
                        <input id="forgetUsername" className="PSignUpInput" onBlur={this.checkEmail} placeholder="Email address" ></input><br />
                        <span id="forgetState" className="PforgetState">{this.state.signUpMessage}</span>
                    </div>
                    <div style={{marginTop:"3%"}} className="PSignUpVerify">
                        <label>Verify Email:</label><br />
                        <input  className="PSignUpCaptcha" maxLength="6" placeholder="code" onChange={this.checkVerification}></input>
                        {
                    ...
                    <div style={this.state.showPassword?{marginTop:"3%"} : {display:"none"}}>
                        <div>
                            ...
                            <input id="forgetPassword" className="PSignUpPassword" type="password" maxLength="30" placeholder="password" onFocus={this.checkPassword} onBlur={this.checkPassword}/><br />
                        </div>
                        <div style={{marginTop:"3%"}}>
                            <label>Confirm Password:</label><span style={{color: "red",fontSize: "14px",marginLeft: "10px"}}>{this.state.passwordSecond}</span><br />
                            <input id="forgetConfirm" className="PSignUpConfirm" type="password" maxLength="30" placeholder="Please confirm your password" onBlur={this.confirmPassword} />
                        </div>
                    </div>
                    
                </div>

                <div style={{marginTop:'4%'}} ><input onClick={this.sendForm} className="PSignUpSubmit" type="button" value="Submit" ></input></div>
